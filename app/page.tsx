...
export default function Home() {
  ...
  const uploadAudio = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    event.target.value = "";
    if (!file || !chat) return;
    const allowedTypes = ['audio/aac', 'audio/mp4', 'audio/mpeg', 'audio/amr', 'audio/ogg', 'audio/opus'];
    if (!allowedTypes.includes(file.type)) {
      setNotice("Selecciona un audio AAC, M4A, MP3, AMR, OGG u OPUS.");
      return;
    }
    if (file.size > 16 * 1024 * 1024) {
      setNotice("El audio no puede superar 16 MB.");
      return;
    }
    setUploadingAudio(true);
    try {
      const response = await fetch(`${api}/conversations/${chat.id}/messages/audio`, {
        method: "POST",
        credentials: "include",
      ...
      setNotice(error instanceof Error ? error.message : "No fue posible enviar el audio.");
    } finally {
    ...
    <main className={view === "inbox" ? "shell" : "shell pipeline-mode"}>
      ...
      {view === "pipeline" ? (
        ...
        <section className="panel">
          ...
          <div className="thread">{!chat && <div className="empty"><b className="mark">M</b><h2>Atiende desde un solo lugar</h2><p>Crea una conversación o espera un mensaje entrante.</p></div>}{messages.map((message) => <article className={message.direction} key={message.id}>{message.type === "audio" && message.media_id ? <audio controls preload="metadata" crossOrigin="use-credentials" src={`${api}/conversations/${chat?.id}/messages/${message.id}/media`}>Tu navegador no puede reproducir este audio.</audio> : message.type === "sticker" && message.media_id ? <img className="sticker" crossOrigin="use-credentials" src={`${api}/conversations/${chat?.id}/messages/${message.id}/media`} alt="Sticker recibido" /> : <p>{message.body || `[${message.type}]`}</p>}<small>{new Date(message.created_at).toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit" })} · {message.status}</small></article>)}</div>
          <form className="composer" onSubmit={send}><label className={uploadingAudio ? "audio-upload loading" : "audio-upload"} aria-label="Enviar audio">{uploadingAudio ? "…" : "♫"}<input type="file" accept="audio/aac,audio/mp4,audio/mpeg,audio/amr,audio/ogg,audio/opus" onChange={uploadAudio} disabled={!chat || uploadingAudio} /></label><textarea disabled={!chat} value={draft} onChange={(event) => setDraft(event.target.value)} placeholder={chat ? "Escribe un mensaje…" : "Selecciona una conversación"} /><button disabled={!chat || !draft.trim()} className="send">↑</button></form>
        </section>
      </>}
