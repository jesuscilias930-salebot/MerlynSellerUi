...
export function Inbox({ chats, chat, messages, number, draft, uploadingAudio, uploadingMedia, documentOptions, selectedDocumentId, onNumberChange, onDraftChange, onCreate, onOpen, onSendText, onUploadAudio, onUploadImage, onUploadVideo, onUploadDocument, onDocumentChange, onSendDocument, onRecordAudio }: Props) {
  return <><section className="list"><header><div><p>BANDEJA</p><h2>Conversaciones</h2></div><b>＋</b></header><form onSubmit={onCreate}><input value={number} onChange={(event) => onNumberChange(event.target.value)} placeholder="Número con código de país" /><button>Nuevo</button></form><div className="rows">{chats.length === 0 && <em>Aún no hay conversaciones.</em>}{chats.map((item) => <button onClick={() => onOpen(item)} className={`${item.id === chat?.id ? "row active" : "row"}${item.unreadCount > 0 ? " has-unread" : ""}`} key={item.id}><b>{initials(item.name || item.phone_number)}</b><span><strong>{item.name || item.phone_number}</strong><small>{item.phone_number} · {item.last_message || "Sin mensajes aún"}</small></span>{item.unreadCount > 0 && <b className="unread-badge" aria-label={`${item.unreadCount} mensajes pendientes`}>{item.unreadCount > 99 ? "99+" : item.unreadCount}</b>}</button>)}</div></section><ConversationPanel chat={chat} messages={messages} draft={draft} uploadingAudio={uploadingAudio} uploadingMedia={uploadingMedia} documentOptions={documentOptions} selectedDocumentId={selectedDocumentId} onDraftChange={onDraftChange} onSendText={onSendText} onUploadAudio={onUploadAudio} onUploadImage={onUploadImage} onUploadVideo={onUploadVideo} onUploadDocument={onUploadDocument} onDocumentChange={onDocumentChange} onSendDocument={onSendDocument} onRecordAudio={onRecordAudio} /></>;
}
