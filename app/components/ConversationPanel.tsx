...
export function ConversationPanel({ chat, messages, draft, uploadingAudio, uploadingMedia, documentOptions, selectedDocumentId, documentCaption, onDraftChange, onSendText, replyToMessage, onReplyToChange, onUploadAudio, onUploadImage, onUploadVideo, onUploadDocument, onDocumentChange, onDocumentCaptionChange, onSendDocument, entrepreneurPackages, quickReplies, stickers, onSendEntrepreneurPackages, onSendSticker, onRecordAudio, onAutoReplyChange, onScenarioChange, automationIntents, onLearnIntent, onDeleteConversation, onClose }: Props) {
  ...
  const bundleImageGroups = entrepreneurPackages.filter((item) => item.controlBundleId).reduce<Record<string, EntrepreneurPackage[]>>((groups, item) => {
    const type = item.bundleType || "Sin tipo";
  ...
  return <section className="panel">
    <header><b className="avatar">{chat ? initials(chat.name || chat.phone_number) : "M"}</b><div><h2>{chat ? chat.name || chat.phone_number : "Tu bandeja está lista"}</h2><small>{chat?.phone_number || "Selecciona un chat para comenzar"}</small></div>{chat && <div className="chat-controls"><button className={`bot-toggle ${chat.autoReplyEnabled !== false ? "on" : ""}`} type="button" onClick={() => onAutoReplyChange(chat.autoReplyEnabled === false)}>{chat.autoReplyEnabled === false ? "Bot apagado" : "Bot activo"}</button><button className={`bot-toggle scenario-toggle ${chat.scenarioEnabled !== false ? "on" : ""}`} type="button" onClick={() => onScenarioChange(chat.scenarioEnabled === false)}>{chat.scenarioEnabled === false ? "Escenarios apagados" : "Escenarios activos"}</button></div>}{chat && <button className="delete-conversation" type="button" onClick={onDeleteConversation}>Borrar prueba</button>}{onClose && <button className="close-conversation" type="button" onClick={onClose} aria-label="Cerrar conversación">×</button>}</header>
    ...
    <button type="button" className="send-bundle-button" disabled={!chat || !Object.keys(bundleImageGroups).length} onClick={() => setBundlePickerOpen(true)}>🎁 Enviar bundle</button>
    {bundlePickerOpen && <div className="bundle-picker-backdrop" role="presentation" onMouseDown={() => setBundlePickerOpen(false)}><section className="bundle-picker-modal" role="dialog" aria-modal="true" aria-label="Enviar bundle" onMouseDown={(event) => event.stopPropagation()}><header><div><p>FOTOGRAFÍAS DE BUNDLES</p><h2>Enviar bundle</h2><span>Elige uno o varios bundles; sus fotos se enviarán seguidas.</span></div><button type="button" onClick={() => setBundlePickerOpen(false)}>×</button></header><div className="bundle-picker-groups">{Object.entries(bundleImageGroups).map(([type, items]) => <section key={type}><h3>{type}</h3>{items.map((item) => <label key={item.id} className={selectedBundleIds.includes(item.id) ? "selected" : ""}>{item.images[0] ? <img src={`${api}/settings/entrepreneur-packages/images/${item.images[0].id}/media`} alt="" /> : <b>▧</b>}<span><strong>{item.name}</strong><small>{item.images.length} fotografía{item.images.length === 1 ? "" : "s"}</small></span><input type="checkbox" checked={selectedBundleIds.includes(item.id)} onChange={() => setSelectedBundleIds((current) => current.includes(item.id) ? current.filter((id) => id !== item.id) : [...current, item.id])} /></label>)}</section>)}</div><footer><button type="button" className="plain-button" onClick={() => setBundlePickerOpen(false)}>Cancelar</button><button type="button" disabled={!selectedBundleIds.length || sendingPackages} onClick={async () => { setSendingPackages(true); try { await onSendEntrepreneurPackages(selectedBundleIds); setSelectedBundleIds([]); setBundlePickerOpen(false); } finally { setSendingPackages(false); } }}>{sendingPackages ? "Enviando…" : "Enviar fotografías"}</button></footer></section></div>}
    <details className="quick-packages">
      <summary>🖼️ Imágenes guardadas</summary>
      {entrepreneurPackages.filter((item) => !item.controlBundleId).length === 0 ? <p className="quick-packages-empty">Aún no hay conjuntos de imágenes guardados.</p> : <>
        <div className="quick-package-list">{entrepreneurPackages.filter((item) => !item.controlBundleId).map((item) => { const images = item.images || []; return <label key={item.id} className={selectedPackageIds.includes(item.id) ? "selected" : ""}>{images[0] ? <img src={`${api}/settings/entrepreneur-packages/images/${images[0].id}/media`} alt="" /> : <b className="image-set-placeholder">▧</b>}<span><strong>{item.name}</strong><small>{images.length} imagen{images.length === 1 ? "" : "es"}</small></span><input type="checkbox" checked={selectedPackageIds.includes(item.id)} onChange={() => setSelectedPackageIds((current) => current.includes(item.id) ? current.filter((id) => id !== item.id) : [...current, item.id])} /></label>; })}</div>
