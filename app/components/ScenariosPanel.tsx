...
function Editor({ initial, columns, reusableMedia, onSave, onDelete, onCancel }: { initial?: AutomationScenario; columns: LeadColumn[]; reusableMedia: ReusableScenarioMedia[]; onSave: Props["onSave"]; onDelete?: () => Promise<void>; onCancel?: () => void }) {
  ...
  const mediaLibrary = [...new Map([...reusableMedia, ...currentScenarioMedia].map((item) => [item.mediaId, item])).values()];
  ...
  return <form className="scenario-editor generic-scenario-editor" onSubmit={submit}>
    ...
    <section className="scenario-steps"><div className="scenario-section-title"><strong>Pasos del flujo</strong><small>Arrastra ⠿ para cambiar el orden. Las conexiones siguen apuntando al mismo paso.</small></div>{value.steps.map((step, index) => <section className={`scenario-step${draggingStepId === step.id ? " dragging" : ""}`} key={step.id} onDragOver={(event) => event.preventDefault()} onDrop={(event) => dropStep(event, step.id)}><div className="scenario-step-heading"><span className="step-drag-handle" draggable title="Arrastra para reordenar este paso" onDragStart={(event) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/merlynsales-scenario-step", step.id); setDraggingStepId(step.id); }} onDragEnd={() => setDraggingStepId(null)}>⠿</span><b>{index + 1}</b><input value={step.label} onChange={(event) => updateStep(step.id, { label: event.target.value })} aria-label="Nombre del paso" /><select value={step.type} onChange={(event) => { const type = event.target.value as ScenarioStep["type"]; updateStep(step.id, { type, body: type === "send_text" ? step.body || "" : undefined, branches: type === "wait_reply" ? step.branches || [] : undefined, items: type === "send_media" ? step.items || [] : undefined }); }}>{Object.entries(labels).map(([type, label]) => <option key={type} value={type}>{label}</option>)}</select><button type="button" className="plain-button" onClick={() => removeStep(step.id)} disabled={value.steps.length <= 1}>Eliminar</button></div>
      ...
      {step.type === "send_media" && <div className="scenario-media"><div className="scenario-media-actions"><label className="media-button">＋ Subir archivo nuevo<input type="file" accept="image/jpeg,image/png,image/webp,application/pdf,.pdf" onChange={(event) => uploadMedia(step, event)} /></label><label className="scenario-media-picker"><span>Reutilizar archivo guardado</span><select defaultValue="" onChange={(event) => { addReusableMedia(step, event.target.value); event.target.value = ""; }} disabled={!mediaLibrary.some((item) => !(step.items || []).some((current) => current.mediaId === item.mediaId))}><option value="">{mediaLibrary.length ? "Selecciona una imagen o documento…" : "Aún no hay archivos en otros escenarios"}</option>{mediaLibrary.filter((item) => !(step.items || []).some((current) => current.mediaId === item.mediaId)).map((item) => <option key={item.mediaId} value={item.mediaId}>{item.type === "document" ? "PDF" : "Imagen"} · {item.filename || "Archivo sin nombre"} · {item.source}</option>)}</select></label></div><small>Elige un archivo ya usado en otro escenario o sube uno nuevo para este paso.</small>{(step.items || []).map((item: ScenarioMedia) => <div className="scenario-asset" key={item.mediaId}><span>{item.type === "document" ? "PDF · " : "Imagen · "}{item.filename || item.mediaId}</span><button type="button" onClick={() => updateStep(step.id, { items: (step.items || []).filter((candidate) => candidate.mediaId !== item.mediaId) })}>Quitar</button></div>)}</div>}
