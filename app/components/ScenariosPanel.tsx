...
const defaultBudgetOptions = (): ScenarioBudgetOption[] => [
  { id: `budget_low_${stepId()}`, label: "Menos de $1,500", min: 0, max: 1499, examples: ["menos de 1500", "mil", "1000", "1200"], packageIds: [], recommendationBody: "Perfecto 🙌🏼 Con ese presupuesto sí tenemos una muy buena opción para comenzar. Le recomendaría {{package_name}} porque incluye buena variedad para iniciar. Le comparto unas fotos para que vea cómo viene 👇🏼" },
  { id: `budget_mid_${stepId()}`, label: "De $1,500 a $3,000", min: 1500, max: 3000, examples: ["1500", "2000", "2500", "3000"], packageIds: [], recommendationBody: "Perfecto 🙌🏼 Con ese presupuesto sí tenemos una muy buena opción para comenzar. Le recomendaría {{package_name}} porque incluye buena variedad para iniciar. Le comparto unas fotos para que vea cómo viene 👇🏼" },
  { id: `budget_high_${stepId()}`, label: "Más de $3,000", min: 3001, examples: ["mas de 3000", "más de 3000", "4000", "5000"], packageIds: [], recommendationBody: "Perfecto 🙌🏼 Con ese presupuesto sí tenemos una muy buena opción para comenzar. Le recomendaría {{package_name}} porque incluye buena variedad para iniciar. Le comparto unas fotos para que vea cómo viene 👇🏼" },
];
...
function FlowBuilder({ initial, columns, entrepreneurPackages, reusableMedia, onSave, onDelete, onCancel }: { initial?: AutomationScenario; columns: LeadColumn[]; entrepreneurPackages: EntrepreneurPackage[]; reusableMedia: ReusableScenarioMedia[]; onSave: Props["onSave"]; onDelete?: () => Promise<void>; onCancel?: () => void }) {
  ...
  const issues = useMemo(() => value.steps.flatMap((step, index) => {
    ...
    if (step.type === "budget_recommendation" && (!(step.budgetOptions || []).length || (step.budgetOptions || []).some((option) => !option.packageIds.length || !option.recommendationBody.trim()))) return [`${prefix}: selecciona paquetes y escribe la recomendación para cada presupuesto.`];
  ...
  const budgetEditor = (step: ScenarioStep) => <div className="flow-branches budget-recommendation-editor"><strong>Paquetes para recomendar según el presupuesto</strong><small>El cliente puede contestar con el rango, una cifra (por ejemplo, “2000”) o una frase similar. Se envía el texto y las fotos de los conjuntos que elijas.</small>{!entrepreneurPackages.length && <p className="scenario-error">Primero crea y carga fotografías en un conjunto reutilizable de paquetes.</p>}{(step.budgetOptions || []).map((option, optionIndex) => <article key={option.id}><input value={option.label} placeholder="Nombre del rango" onChange={(event) => { const budgetOptions = [...(step.budgetOptions || [])]; budgetOptions[optionIndex] = { ...option, label: event.target.value }; updateStep(step.id, { budgetOptions }); }} /><div className="budget-range-fields"><label>Mínimo<input type="number" min="0" value={option.min ?? ""} onChange={(event) => { const budgetOptions = [...(step.budgetOptions || [])]; budgetOptions[optionIndex] = { ...option, min: event.target.value === "" ? undefined : Number(event.target.value) }; updateStep(step.id, { budgetOptions }); }} /></label><label>Máximo<input type="number" min="0" value={option.max ?? ""} placeholder="Sin límite" onChange={(event) => { const budgetOptions = [...(step.budgetOptions || [])]; budgetOptions[optionIndex] = { ...option, max: event.target.value === "" ? undefined : Number(event.target.value) }; updateStep(step.id, { budgetOptions }); }} /></label></div><label>Paquetes o conjuntos de fotos a enviar<select multiple value={option.packageIds} onChange={(event) => { const budgetOptions = [...(step.budgetOptions || [])]; budgetOptions[optionIndex] = { ...option, packageIds: Array.from(event.target.selectedOptions, (item) => item.value) }; updateStep(step.id, { budgetOptions }); }}>{entrepreneurPackages.map((item) => <option key={item.id} value={item.id}>{item.name}{item.images.length ? ` · ${item.images.length} fotos` : " · sin fotos"}</option>)}</select><small>Mantén ⌘ en Mac o Ctrl en Windows para elegir varios.</small></label><textarea value={option.recommendationBody} placeholder="Mensaje de recomendación" onChange={(event) => { const budgetOptions = [...(step.budgetOptions || [])]; budgetOptions[optionIndex] = { ...option, recommendationBody: event.target.value }; updateStep(step.id, { budgetOptions }); }} /><small>Variables disponibles: <code>{"{{package_name}}"}</code>, <code>{"{{package_names}}"}</code> y <code>{"{{budget}}"}</code>.</small></article>)}</div>;
