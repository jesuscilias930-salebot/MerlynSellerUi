...
type Props = { scenarios: AutomationScenario[]; columns: LeadColumn[]; entrepreneurPackages: EntrepreneurPackage[]; onSave: (value: Draft, id?: string) => Promise<void>; onDelete: (id: string) => Promise<void>; onReorder: (scenarioIds: string[]) => Promise<void> };
...
const labels: Record<ScenarioStep["type"], string> = { send_text: "Enviar mensaje", send_catalog: "Enviar catálogo", send_media: "Enviar imágenes o documentos", wait_reply: "Esperar respuesta", budget_recommendation: "Recomendar por presupuesto", move_column: "Mover lead", end: "Finalizar" };
const icons: Record<ScenarioStep["type"], string> = { send_text: "✉", send_catalog: "▤", send_media: "▧", wait_reply: "◌", budget_recommendation: "💰", move_column: "↗", end: "✓" };
const defaultBudgetOptions = (): ScenarioBudgetOption[] => [
  { id: `budget_low_${stepId()}`, label: "Menos de $1,500", min: 0, max: 1499, examples: ["menos de 1500", "mil", "1000", "1200"], packageIds: [], items: [], recommendationBody: "Perfecto 🙌🏼 Con ese presupuesto sí tenemos una muy buena opción para comenzar. Le recomendaría {{package_name}} porque incluye buena variedad para iniciar. Le comparto unas fotos para que vea cómo viene 👇🏼" },
  { id: `budget_mid_${stepId()}`, label: "De $1,500 a $3,000", min: 1500, max: 3000, examples: ["1500", "2000", "2500", "3000"], packageIds: [], items: [], recommendationBody: "Perfecto 🙌🏼 Con ese presupuesto sí tenemos una muy buena opción para comenzar. Le recomendaría {{package_name}} porque incluye buena variedad para iniciar. Le comparto unas fotos para que vea cómo viene 👇🏼" },
  { id: `budget_high_${stepId()}`, label: "Más de $3,000", min: 3001, examples: ["mas de 3000", "más de 3000", "4000", "5000"], packageIds: [], items: [], recommendationBody: "Perfecto 🙌🏼 Con ese presupuesto sí tenemos una muy buena opción para comenzar. Le recomendaría {{package_name}} porque incluye buena variedad para iniciar. Le comparto unas fotos para que vea cómo viene 👇🏼" },
];
const blankStep = (type: ScenarioStep["type"] = "send_text"): ScenarioStep => ({ id: stepId(), type, label: labels[type], ...(type === "send_text" ? { body: "" } : {}), ...(type === "wait_reply" ? { branches: [] } : {}), ...(type === "budget_recommendation" ? { budgetOptions: defaultBudgetOptions() } : {}) });
...
function FlowBuilder({ initial, columns, entrepreneurPackages, reusableMedia, onSave, onDelete, onCancel }: { initial?: AutomationScenario; columns: LeadColumn[]; entrepreneurPackages: EntrepreneurPackage[]; reusableMedia: ReusableScenarioMedia[]; onSave: Props["onSave"]; onDelete?: () => Promise<void>; onCancel?: () => void }) {
  ...
  const issues = useMemo(() => value.steps.flatMap((step, index) => {
    ...
    if (step.type === "budget_recommendation" && (!(step.budgetOptions || []).length || (step.budgetOptions || []).some((option) => (!(option.packageIds || []).length && !(option.items || []).length) || !option.recommendationBody.trim()))) return [`${prefix}: agrega fotografías o paquetes y escribe la recomendación para cada presupuesto.`];
    if (step.type === "budget_recommendation" && !step.nextStepId) return [`${prefix}: elige el paso que continúa después de recomendar.`];
  ...
  const addStep = () => {
    ...
    setValue((current) => {
      ...
      const steps = [...current.steps, step].map((candidate) => selected && candidate.id === selected.id && !selected.nextStepId && !["wait_reply", "budget_recommendation", "end"].includes(selected.type) ? { ...candidate, nextStepId: step.id } : candidate);
  ...
  const saveScenario = async (closeSettings = false) => {
    ...
    try {
      await onSave({ ...value, aiDescription: value.aiDescription?.trim() || null, triggerExamples: triggerText.split("\n").map((item) => item.trim()).filter(Boolean), steps: value.steps.map((step) => ({ ...step, branches: step.branches?.map((branch) => ({ ...branch, name: branch.name.trim(), aiDescription: branch.aiDescription?.trim() || undefined, examples: branch.examples.map((item) => item.trim()).filter(Boolean) })) })) }, initial?.id);
  ...
  const submit = (event: FormEvent) => { event.preventDefault(); void saveScenario(); };
  ...
  const updateBudgetOption = (step: ScenarioStep, optionId: string, patch: Partial<ScenarioBudgetOption>) => updateStep(step.id, { budgetOptions: (step.budgetOptions || []).map((option) => option.id === optionId ? { ...option, ...patch } : option) });
  const budgetImageLibrary = useMemo(() => {
    const savedImages: ReusableScenarioMedia[] = entrepreneurPackages.flatMap((item) => item.images.map((image) => ({ mediaId: image.mediaId, filename: image.filename, caption: image.caption || item.caption || undefined, type: "image", source: `Imágenes guardadas · ${item.name}` })));
    return [...new Map([...savedImages, ...mediaLibrary.filter((item) => item.type !== "document")].map((item) => [item.mediaId, item])).values()];
  }, [entrepreneurPackages, mediaLibrary]);
  const addBudgetImage = (step: ScenarioStep, option: ScenarioBudgetOption, mediaId: string) => {
    const item = budgetImageLibrary.find((candidate) => candidate.mediaId === mediaId);
    if (!item || (option.items || []).some((candidate) => candidate.mediaId === item.mediaId)) return;
    updateBudgetOption(step, option.id, { items: [...(option.items || []), { mediaId: item.mediaId, filename: item.filename, caption: item.caption, type: "image" }] });
  };
  const uploadBudgetImage = async (step: ScenarioStep, option: ScenarioBudgetOption, event: ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0]; event.target.value = ""; if (!file) return;
    ...
    try {
      const response = await fetch(`${api}/scenarios/evidence/upload`, { method: "POST", credentials: "include", headers: { "Content-Type": file.type, "X-Upload-Filename": encodeURIComponent(file.name) }, body: file });
      const uploaded = (await response.json().catch(() => ({}))) as { error?: string; mediaId?: string; filename?: string };
      if (!response.ok || !uploaded.mediaId) throw new Error(uploaded.error || "No fue posible subir la imagen.");
      updateBudgetOption(step, option.id, { items: [...(option.items || []), { mediaId: uploaded.mediaId, filename: uploaded.filename || file.name, type: "image" }] });
    } catch (reason) { setError(reason instanceof Error ? reason.message : "No fue posible subir la imagen."); }
  };
  const budgetEditor = (step: ScenarioStep) => <div className="flow-branches budget-recommendation-editor"><strong>Paquetes para recomendar según el presupuesto</strong><small>Agrega tantos rangos como necesites. Cada respuesta envía su texto, los paquetes elegidos y las fotografías individuales que configures.</small>{(step.budgetOptions || []).map((option) => <article key={option.id}><div className="budget-option-header"><strong>Rango de presupuesto</strong><button type="button" className="danger-link" onClick={() => updateStep(step.id, { budgetOptions: (step.budgetOptions || []).filter((candidate) => candidate.id !== option.id) })}>Eliminar rango</button></div><input value={option.label} placeholder="Nombre del rango" onChange={(event) => updateBudgetOption(step, option.id, { label: event.target.value })} /><div className="budget-range-fields"><label>Mínimo<input type="number" min="0" value={option.min ?? ""} onChange={(event) => updateBudgetOption(step, option.id, { min: event.target.value === "" ? undefined : Number(event.target.value) })} /></label><label>Máximo<input type="number" min="0" value={option.max ?? ""} placeholder="Sin límite" onChange={(event) => updateBudgetOption(step, option.id, { max: event.target.value === "" ? undefined : Number(event.target.value) })} /></label></div><label>Paquetes o conjuntos de fotos<select multiple value={option.packageIds || []} onChange={(event) => updateBudgetOption(step, option.id, { packageIds: Array.from(event.target.selectedOptions, (item) => item.value) })}>{entrepreneurPackages.map((item) => <option key={item.id} value={item.id}>{item.name}{item.images.length ? ` · ${item.images.length} fotos` : " · sin fotos"}</option>)}</select><small>Los paquetes envían todas sus fotos. Mantén ⌘ en Mac o Ctrl en Windows para elegir varios.</small></label><div className="budget-photo-picker"><label>Fotografías individuales<select defaultValue="" onChange={(event) => { addBudgetImage(step, option, event.target.value); event.target.value = ""; }}><option value="">Selecciona una imagen guardada…</option>{budgetImageLibrary.filter((item) => !(option.items || []).some((selected) => selected.mediaId === item.mediaId)).map((item) => <option key={item.mediaId} value={item.mediaId}>{item.filename || "Imagen"} · {item.source}</option>)}</select></label><label className="media-button">＋ Cargar fotografía nueva<input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => void uploadBudgetImage(step, option, event)} /></label>{(option.items || []).length > 0 && <div className="budget-selected-photos">{option.items?.map((item) => <span key={item.mediaId}>▧ {item.filename || "Imagen"}<button type="button" onClick={() => updateBudgetOption(step, option.id, { items: (option.items || []).filter((candidate) => candidate.mediaId !== item.mediaId) })} aria-label={`Quitar ${item.filename || "imagen"}`}>×</button></span>)}</div>}</div><textarea value={option.recommendationBody} placeholder="Mensaje de recomendación" onChange={(event) => updateBudgetOption(step, option.id, { recommendationBody: event.target.value })} /><small>Variables disponibles: <code>{"{{package_name}}"}</code>, <code>{"{{package_names}}"}</code> y <code>{"{{budget}}"}</code>.</small></article>)}<button type="button" className="plain-button" onClick={() => updateStep(step.id, { budgetOptions: [...(step.budgetOptions || []), { id: `budget_${stepId()}`, label: "", examples: [], packageIds: [], items: [], recommendationBody: "" }] })}>＋ Agregar rango de presupuesto</button></div>;
...
export function ScenariosPanel({ scenarios, columns, entrepreneurPackages, onSave, onDelete, onReorder }: Props) {
