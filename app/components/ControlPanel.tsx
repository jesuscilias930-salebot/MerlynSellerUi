...
export function ControlPanel({ chats }: { chats: Chat[] }) {
  ...
  if (!token) return <section className="control-login"><div><p>CONTROL DE VENTAS</p><h1>Conecta tu sistema de ventas</h1><span>Ambos backends siguen separados. Inicia sesión con las credenciales de Sock Control.</span><small>API configurada: {controlApi}</small></div><form onSubmit={login}><input value={identifier} onChange={(event) => setIdentifier(event.target.value)} placeholder="Usuario o correo" required /><input value={password} onChange={(event) => setPassword(event.target.value)} type="password" placeholder="Contraseña" required /><button disabled={loading}>{loading ? "Conectando…" : "Conectar"}</button>{notice && <em>{notice}</em>}</form></section>;
